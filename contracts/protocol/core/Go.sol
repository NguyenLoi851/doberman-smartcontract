...
contract Go is IGo, BaseUpgradeablePausable {
    ...
    function go(address account) public view override returns (bool) {
        ...
        if (
            ...
            IUniqueIdentity0612(uniqueIdentity).balanceOf(account, ID_TYPE_0) >
            0
        ) {
            return true;
        }
        ...
        for (uint256 i = 1; i < allIdTypes.length; ++i) {
            uint256 idTypeBalance = IUniqueIdentity0612(uniqueIdentity)
                .balanceOf(account, allIdTypes[i]);
            if (idTypeBalance > 0) {
                return true;
            }
        }
    ...
    function goOnlyIdTypes(
        ...
        for (uint256 i = 0; i < onlyIdTypes.length; ++i) {
            ...
            uint256 idTypeBalance = IUniqueIdentity0612(uniqueIdentity)
                .balanceOf(account, onlyIdTypes[i]);
            if (idTypeBalance > 0) {
                return true;
            }
        }
    ...
    function goSeniorPool(address account) public view override returns (bool) {
        ...
        for (uint256 i = 0; i < seniorPoolIdTypes.length; ++i) {
            uint256 idTypeBalance = IUniqueIdentity0612(uniqueIdentity)
                .balanceOf(account, seniorPoolIdTypes[i]);
            if (idTypeBalance > 0) {
                return true;
            }
        }
