    ...
    SafeERC20Transfer
{
    ...
    function initialize(
        ...
        bool success = config.getUSDC().approve(
            address(this),
            type(uint256).max
        );
    ...
    function setAllowedUIDTypes(uint256[] calldata ids) public onlyLocker {
        require(
            poolSlices[0].juniorTranche.principalDeposited == 0 &&
                poolSlices[0].seniorTranche.principalDeposited == 0,
    ...
    function deposit(
        uint256 tranche,
        uint256 amount
    ) public override nonReentrant whenNotPaused returns (uint256 tokenId) {
        ...
        require(
            trancheInfo.principalDeposited.add(amount) <= creditLine.maxLimit(),
            "Exceed max limit of pool"
        );
        trancheInfo.principalDeposited = trancheInfo.principalDeposited.add(
            amount
        );
    ...
    function withdrawMultiple(
        ...
        require(
            ...
            "TokensIds and Amounts mismatch"
        );
    ...
    function drawdown(
        ...
        currentSlice.seniorTranche.principalSharePrice = currentSlice
            .seniorTranche
            .calculateExpectedSharePrice(amountRemaining, currentSlice);
        currentSlice.principalDeployed = currentSlice.principalDeployed.add(
            amount
        );
        ...
        emit SharePriceUpdated(
            ...
            currentSlice.juniorTranche.principalSharePrice,
            int256(
                oldJuniorPrincipalSharePrice.sub(
                    currentSlice.juniorTranche.principalSharePrice
                )
            ) * -1,
        ...
        emit SharePriceUpdated(
            ...
            currentSlice.seniorTranche.principalSharePrice,
            int256(
                oldSeniorPrincipalSharePrice.sub(
                    currentSlice.seniorTranche.principalSharePrice
                )
            ) * -1,
    ...
    function getTranche(
        uint256 tranche
    ) public view override returns (TrancheInfo memory) {
    ...
    function availableToWithdrawMultiple(
        uint256[] calldata tokenIds
    )
        external
        view
        returns (
            uint256 interestRedeemableTotal,
            uint256 principalRedeemableTotal
        )
    {
        require(tokenIds.length > 0, "Do not have any element");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            (
                uint256 interestRedeemable,
                uint256 principalRedeemable
            ) = availableToWithdraw(tokenIds[i]);
    ...
    function _withdraw(
        ...
        if (trancheInfo.lockedUntil == 0) {
            trancheInfo.principalDeposited = trancheInfo.principalDeposited.sub(
                amount
            );
        }

        uint256 interestToRedeem = Math.min(interestRedeemable, amount);
        uint256 principalToRedeem = Math.min(
            principalRedeemable,
            amount.sub(interestToRedeem)
        );

        config.getPoolTokens().redeem(
            tokenId,
            principalToRedeem,
            interestToRedeem
        );
        ...
        returns (uint256 interestRedeemable, uint256 principalRedeemable)
    {
        ...
        if (
            (timestamp >
                creditLine.termEndTime() +
                    creditLine.paymentPeriodInDays().mul(SECONDS_PER_DAY)) &&
            creditLine.balance() > 0 &&
            (creditLine.auctionEnd() == 0 ||
                timestamp < creditLine.auctionEnd())
        ) {
            return (0, 0);
        }
        ...
        interestRedeemable = maxInterestRedeemable.sub(
            tokenInfo.interestRedeemed
        );
        principalRedeemable = maxPrincipalRedeemable.sub(
    ...
    function _lockJuniorCapital(uint256 sliceId) internal {
        ...
        require(
            poolSlices[sliceId].juniorTranche.lockedUntil == 0,
            "Junior tranche already locked"
        );

        uint256 lockedUntil = currentTime().add(
    ...
    function _lockPool() internal {
        ...
        require(
            poolSlices[sliceId].seniorTranche.lockedUntil == 0,
            "Lock cannot be extended"
        );

        uint256 currentTotal = poolSlices[sliceId]
            .juniorTranche
            .principalDeposited
            .add(poolSlices[sliceId].seniorTranche.principalDeposited);
        creditLine.setLimit(
            Math.min(
                creditLine.limit().add(currentTotal),
                creditLine.maxLimit()
            )
        );
        ...
        poolSlices[sliceId].seniorTranche.lockedUntil = currentTime().add(
            lockPeriod
        );
        poolSlices[sliceId].juniorTranche.lockedUntil = currentTime().add(
            lockPeriod
        );
        emit TrancheLocked(
            address(this),
    ...
    function collectInterestAndPrincipal(
        ...
        uint256 reserveFeePercent = ONE_HUNDRED.div(
        ...
        sendToSeniorPool(
            interest - result.interestRemaining,
            principal - result.principalRemaining
        );
    ...
    function locked() internal view returns (bool) {
        return
            poolSlices[poolSlices.length.sub(1)].seniorTranche.lockedUntil > 0;
    }
    ...
    function getTrancheInfo(
        uint256 trancheId
    ) internal view returns (TrancheInfo storage) {
        require(
            trancheId > 0 &&
                trancheId <= poolSlices.length.mul(NUM_TRANCHES_PER_SLICE),
            "Unsupported tranche"
        );
        uint256 sliceId = (
            (trancheId.add(trancheId.mod(NUM_TRANCHES_PER_SLICE))).div(
                NUM_TRANCHES_PER_SLICE
            )
        ).sub(1);
        PoolSlice storage slice = poolSlices[sliceId];
        TrancheInfo storage trancheInfo = trancheId.mod(
            NUM_TRANCHES_PER_SLICE
        ) == 1
            ? slice.seniorTranche
            : slice.juniorTranche;
    ...
    function sendToSeniorPool(uint256 interest, uint256 principal) internal {
        ...
        (bool success, ) = seniorPool.call(
            abi.encodeWithSignature(
                "collectInterestAndPrincipal(uint256,uint256)",
                interest,
                principal
            )
        );
    ...
    function _assess() internal {
        ...
        require(
            currentTime() <=
                creditLine.termEndTime() +
                    creditLine.paymentPeriodInDays().mul(SECONDS_PER_DAY),
            "Late to repay, asset is auctioned"
        );
        ...
        interestAccrued = creditLine.totalInterestAccrued().sub(
            interestAccrued
        );
        ...
        // linearly
        uint256[] memory principalPaymentsPerSlice = new uint256[](
            poolSlices.length
        );
        for (uint256 i = 0; i < poolSlices.length; i++) {
            ...
            poolSlices[i].totalInterestAccrued = poolSlices[i]
                .totalInterestAccrued
                .add(interestForSlice);
        }
        ...
        if (interestPayment > 0 || principalPayment > 0) {
            ...
            for (uint256 i = 0; i < poolSlices.length; i++) {
                poolSlices[i].principalDeployed = poolSlices[i]
                    .principalDeployed
                    .sub(principalPaymentsPerSlice[i]);
