...
library TranchingLogic {
    ...
    function usdcToSharePrice(
        ...
        return
            totalShares == 0
                ? 0
                : amount.mul(FP_SCALING_FACTOR).div(totalShares);
    }
    ...
    function getTotalInterestAndPrincipal(
        ...
        principalAccrued = totalDeployed.sub(creditLine.balance()).add(
            principalAccrued
        );
        ...
        principalAccrued = totalDeposited.sub(slice.principalDeployed).add(
            principalAccrued
        );
    ...
    function scaleByFraction(
        ...
        FixedPoint.Unsigned memory totalAsFixedPoint = FixedPoint
            .fromUnscaledUint(total);
        FixedPoint.Unsigned memory fractionAsFixedPoint = FixedPoint
    ...
    function emitSharePriceUpdatedEvent(
        ...
        emit SharePriceUpdated(
            ...
            int256(
                tranche.principalSharePrice.sub(
                    applyResult.oldPrincipalSharePrice
                )
            ),
            tranche.interestSharePrice,
            int256(
                tranche.interestSharePrice.sub(
                    applyResult.oldInterestSharePrice
                )
            )
        );
    }
    ...
    function applyToSharePrice(
        ...
        return (
            amountRemaining.sub(desiredAmount),
            currentSharePrice.add(sharePriceDifference)
        );
    }
