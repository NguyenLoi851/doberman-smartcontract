...
contract PoolTokens is IPoolTokens, ERC721PresetMinterPauserAutoIdUpgradeSafe {
  ...
  */
    ...
    function redeem(
        ...
        pool.totalPrincipalRedeemed = pool.totalPrincipalRedeemed.add(
            principalRedeemed
        );
        require(
            pool.totalPrincipalRedeemed <= pool.totalMinted,
            "Cannot redeem more than we minted"
        );

        token.principalRedeemed = token.principalRedeemed.add(
            principalRedeemed
        );
        require(
    ...
    function burn(uint256 tokenId) external virtual override whenNotPaused {
        ...
        bool fromTokenPool = _validPool(_msgSender()) &&
            token.pool == _msgSender();
        address owner = ownerOf(tokenId);
        require(
            canBurn || fromTokenPool,
    ...
    function onPoolCreated(
        address newPool
    ) external override onlyDobermanFactory {
    ...
    modifier onlyDobermanFactory() {
        require(
            _msgSender() == config.DobermanFactoryAddress(),
            "Only Doberman factory is allowed"
        );
        _;
    }
