...
abstract contract LeverageRatioStrategy is
    ...
    ISeniorPoolStrategy
{
    ...
    function _invest(
        ...
        uint256 seniorTarget = juniorCapital.mul(getLeverageRatio(pool)).div(
            LEVERAGE_RATIO_DECIMALS
        );

        IV2CreditLine creditLine = pool.creditLine();
        uint256 maxLimit = creditLine.maxLimit();
        seniorTarget = (juniorCapital + seniorTarget > maxLimit)
            ? maxLimit - juniorCapital
            : seniorTarget;
