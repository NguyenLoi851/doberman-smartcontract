...
contract Pool is BaseUpgradeablePausable, IPool {
    ...
    event DepositMade(
        address indexed capitalProvider,
        uint256 amount,
        uint256 shares
    );
        ...
        nonReentrant
    {
        ...
        require(
            poolWithinLimit(potentialNewTotalShares),
            "Deposit would put the Pool over the total limit."
        );
    ...
    function assets() public view override returns (uint256) {
        ...
        return
            compoundBalance
                ...
                .add(creditDesk.totalLoansOutstanding())
                .sub(creditDesk.totalWritedowns());
    }
    ...
    function migrateToSeniorPool() external onlyAdmin {
        ...
        // Claim our COMP!
        address compoundController = address(
    ...
    function _withdraw(
    ...
    ) internal withinTransactionLimit(usdcAmount) {
        ...
        uint256 reserveAmount = usdcAmount.div(
    ...
    function poolWithinLimit(
        uint256 _totalShares
    ) internal view returns (bool) {
        return
            _totalShares.mul(sharePrice).div(fiduMantissa()) <=
            usdcToFidu(
                config.getNumber(uint256(ConfigOptions.Numbers.TotalFundsLimit))
            );
    }

    function transactionWithinLimit(
        uint256 amount
    ) internal view returns (bool) {
        return
            amount <=
            config.getNumber(uint256(ConfigOptions.Numbers.TransactionLimit));
    }
    ...
    modifier onlyCreditDesk() {
        require(
            ...
            "Only the credit desk is allowed to call this function"
        );
        _;
    }
}
