...
contract CreditLine is BaseUpgradeablePausable, ICreditLine, SafeERC20Transfer {
    ...
    function drawdown(uint256 amount) external onlyAdmin {
        require(
            amount.add(balance) <= currentLimit,
        ...
        ) = updateAndGetInterestAndPrincipalOwedAsOf(timestamp);
        ...
        returns (bool)
    {
        ...
        return
            block.timestamp <
            _termStartTime().add(
                principalGracePeriodInDays.mul(SECONDS_PER_DAY)
            );
    }
    ...
    function setTotalInterestAccrued(
    ...
    function setInterestAccruedAsOf(
        uint256 newInterestAccruedAsOf
    ) public onlyAdmin {
        interestAccruedAsOf = newInterestAccruedAsOf;
    }

    function setLastFullPaymentTime(
        uint256 newLastFullPaymentTime
    ) public onlyAdmin {
    ...
    function calculateNextDueTime() public view returns (uint256) {
        ...
        if (balance > 0 && curTimestamp >= newNextDueTime) {
            uint256 secondsToAdvance = (
                curTimestamp.sub(newNextDueTime).div(secondsPerPeriod)
            ).add(1).mul(secondsPerPeriod);
    ...
    function _isLate(uint256 timestamp) internal view returns (bool) {
        uint256 secondsElapsedSinceFullPayment = timestamp.sub(
            lastFullPaymentTime
        );
        return
            balance > 0 &&
            secondsElapsedSinceFullPayment >
            paymentPeriodInDays.mul(SECONDS_PER_DAY);
    }
    ...
    function handlePayment(
        ...
        uint256 paymentRemaining = paymentAmount.sub(pa.interestPayment).sub(
        ...
        assert(
            paymentRemaining.add(pa.interestPayment).add(
                totalPrincipalPayment
            ) == paymentAmount
        );
    ...
    function updateAndGetInterestAndPrincipalOwedAsOf(
        uint256 timestamp
        ...
        return (
            interestOwed.add(interestAccrued),
            principalOwed.add(principalAccrued)
        );
    }

    function getInterestAndPrincipalOwedAsOfCurrent()
        public
        view
    ...
    function updateCreditLineAccounting(
        ...
        if (newInterestOwed == 0 && _nextDueTime != 0) {
            ...
            if (currentTime() < _nextDueTime) {
                uint256 secondsPerPeriod = paymentPeriodInDays.mul(
                    SECONDS_PER_DAY
                );
    ...
    function bidWithPermit(
        ...
        require(
            (timestamp >
                termEndTime + paymentPeriodInDays.mul(SECONDS_PER_DAY)) &&
                balance > 0,
            "Loans is on time or fully repaid"
        );

        // start bid
        if (auctionEnd == 0) {
            _setAuctionEnd(
                timestamp.add(SECONDS_PER_DAY.mul(paymentPeriodInDays))
            );
            (
                uint256 interestOwe,
                uint256 principalOwe
            ) = getInterestAndPrincipalOwedAsOfCurrent();
            require(
                amount >= interestOwe + principalOwe,
                "Start auction bid too low"
            );
        } else {
            require(timestamp <= auctionEnd, "Auction end");
            uint256 increase = config.getMinBidIncrease() + 10000;
            require(
                amount * 10000 >= auctionLivePrice * increase,
                "Bid too low"
            );
        }
