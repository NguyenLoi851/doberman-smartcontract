...
library Accountant {
    ...
    function calculateInterestAccruedOverPeriod(
        ...
        uint256 totalInterestPerYear = balance.mul(cl.interestApr()).div(
            INTEREST_DECIMALS
        );
        interestOwed = totalInterestPerYear.mul(secondsElapsed).div(
            SECONDS_PER_YEAR
        );
        if (lateFeeApplicable(cl, endTime, lateFeeGracePeriodInDays)) {
            uint256 lateFeeInterestPerYear = balance.mul(cl.lateFeeApr()).div(
                INTEREST_DECIMALS
            );
            uint256 additionalLateFeeInterest = lateFeeInterestPerYear
                .mul(secondsElapsed)
    ...
    function lateFeeApplicable(
        ...
        return
            cl.lateFeeApr() > 0 &&
            secondsLate > gracePeriodInDays.mul(SECONDS_PER_DAY);
    }
    ...
    function allocatePayment(
        ...
        uint256 additionalBalancePayment = Math.min(
            paymentRemaining,
            balanceRemaining
        );
